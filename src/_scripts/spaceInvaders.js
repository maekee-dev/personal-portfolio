...
console.log('-> loading spaceInvaders.js')

const canvas = document.querySelector('.game-canvas canvas')
canvas.width = document.querySelector('.console-section .console-body').offsetWidth
canvas.height = document.querySelector('.console-section .console-body').offsetHeight
...
const startGame = () => {
    gsap.to('.page-menu', {display: 'none', opacity: 0})
    if(!window.matchMedia("(any-pointer: fine)").matches){
        gsap.to('.game-errors', {display: 'flex'})
        return null
    }
    const gameCountdown = document.getElementById('game-countdown')
    gsap.to('.game-overlay', {display: 'flex'})
    ...
    setTimeout(() => {
        gsap.to('.game-canvas', { display: 'flex' })
        gsap.to('.game-overlay', {display: 'none'})
        animate()
    }, 4000);   
    for(let i = 0; i < 100; i++){
        particles.push(new Particle({
